...
class SelfImprovementEngine:
    """
    Engine for analyzing agent performance and generating improvement strategies.
    """
    
    def __init__(self, memory_system: MemorySystem, llm_provider):
        self.memory = memory_system
        self.llm_provider = llm_provider
    
    async def run_improvement_cycle(self) -> Dict[str, Any]:
        """
        Analyze recent task history, identify failures, and generate new strategies.
        """
        # 1. Analyze recent failures
        failures = self.memory.get_failure_analysis()
        
        # 2. Analyze successful patterns
        successes = self.memory.get_successful_patterns()
        
        # 3. Use LLM to generate insights
        prompt = f"""
        Analyze the following agent performance data:
        
        Recent Failures:
        {json.dumps(failures, default=str, indent=2)}
        
        Successful Patterns:
        {json.dumps(successes[:5], default=str, indent=2)}
        
        Generate 1-3 specific strategies to improve performance.
        Return ONLY valid JSON with keys: 'strategies' (list of strings), 'lessons' (list of strings).
        ...
        try:
            ...
            if not response.get('success', False):
                logger.warning(f"LLM generation failed: {response.get('error')}")
                return {"strategies_created": [], "lessons_extracted": []}
            ...
            logger.error(f"Error parsing improvement strategies: {e}")
            strategies = []
            lessons = ["Failed to parse improvement strategies"]

        # 4. Store new strategies
        created_strategies = []
        for strat in strategies:
            self.memory.create_strategy(strat, "Generated by SelfImprovementEngine", "Improvement")
            created_strategies.append(strat)
            
        # 5. Store lessons
        for lesson in lessons:
            # Use a dummy ID for the general lesson
            self.memory.learn_lesson("general_improvement", lesson)
